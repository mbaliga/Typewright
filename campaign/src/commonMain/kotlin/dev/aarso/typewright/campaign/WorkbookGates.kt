...
import dev.aarso.typewright.qa.checkAlignmentMiss
import dev.aarso.typewright.qa.checkAnchorsPresent
import dev.aarso.typewright.qa.checkCollinearSegments
import dev.aarso.typewright.qa.checkExtremaOnCurve
import dev.aarso.typewright.qa.checkJaggyTurns
import dev.aarso.typewright.qa.checkNodeEconomy
import dev.aarso.typewright.qa.checkOvershootPresence
import dev.aarso.typewright.qa.checkSemiVertical
import dev.aarso.typewright.qa.checkShortSegments
import dev.aarso.typewright.qa.corpus.NodeEconomyCorpus
import dev.aarso.typewright.qa.corpus.NodeEconomyVerdict
import dev.aarso.typewright.qa.ship.generateDescriptionHtml
import dev.aarso.typewright.qa.ship.generateMetadataPbText
import dev.aarso.typewright.qa.ship.generateOflText
import dev.aarso.typewright.qa.ship.gfCopyrightLine
 ...
 * - [dev.aarso.typewright.qa.LayerOneChecker] (task 11) already speaks
 *   [dev.aarso.typewright.qa.CheckStatus.WARN] for real, and a check firing only more than 100
...
public data class WorkbookGateResult(
    ...
    /** How many [checks] entries fall into each [GateCheckStatus], for a summary line (mirrors [dev.aarso.typewright.qa.LayerOneCounts]). */
...
public object WorkbookGates {
     ...
     * documented contract: "returns an honest, empty-ish [dev.aarso.typewright.qa.LayerOneReport]
