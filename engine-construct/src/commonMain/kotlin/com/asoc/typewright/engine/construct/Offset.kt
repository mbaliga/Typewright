...
/** The signed angle in radians, in `(-PI, PI]`, from unit vector [a] to unit vector [b] (`atan2(cross, dot)`) — the same convention as `core-geometry`'s [com.asoc.typewright.core.geometry.angleBetween], restated here over [Vec2] directly since that one is expressed over integer [Point]-based [com.asoc.typewright.core.geometry.Segment]s. */
