...
import dev.aarso.typewright.learn.scenes.ExerciseBankEntry
import dev.aarso.typewright.learn.scenes.LearnFaceResources
import dev.aarso.typewright.learn.scenes.StrandExercisePlan
 ...
 * no Compose runtime required — the same split [dev.aarso.typewright.ui.sheet.SheetCamera] and
 ...
 * [dev.aarso.typewright.learn.scenes.StrandSequencer.plan] actually returns, not a paraphrase of
 ...
 * (e.g. `"librebodoni"`) — [dev.aarso.typewright.learn.scenes.Scene.faces]/[LearnFaceEntry] both
 ...
 * not necessarily in the current [dev.aarso.typewright.learn.scenes.Scene.faces] list at all.
 *
...
/** [t] clamped to `0.0..1.0` — [dev.aarso.typewright.learn.scenes.SceneRenderer]'s own scrub contract. */
internal fun clampScrub(t: Double): Double = t.coerceIn(0.0, 1.0)

/** [index] clamped to a valid position in a list of [sceneCount] scenes (never empty in practice — [sceneCount] is [dev.aarso.typewright.learn.scenes.LineagesResources.loadFullBlockInPlayOrder]'s own fixed eleven). */
