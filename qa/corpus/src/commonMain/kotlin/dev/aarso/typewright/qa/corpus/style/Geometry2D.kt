 ...
 * not from at-rest [dev.aarso.typewright.core.geometry.Point]s).
 *
 ...
 * [dev.aarso.typewright.core.geometry.Extremum] the contour has (via [extrema], `core-geometry`'s
...
/** [contour]'s points, resampled to [perSegment] evenly-`t`-spaced points per [Contour.segments] segment, in contour order. Used by [narrowestThroat] and [dev.aarso.typewright.qa.corpus.style.superellipseExponent] as a cheap stand-in for arc-length sampling -- uniform in curve parameter, not in arc length, which is an acceptable approximation for this package's coarse ratios. */
 ...
 * filter alone is kept as the fallback) so [dev.aarso.typewright.qa.corpus.style.superellipseExponent]'s
