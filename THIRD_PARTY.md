...
| kotlinx.serialization JSON (`org.jetbrains.kotlinx:kotlinx-serialization-json`, `-json-jvm`, `-json-wasm-js`; pulls in `kotlinx-serialization-core`, `-core-jvm`, `-core-wasm-js`), used by `qa:corpus`'s node-economy data loader (P1) and, directly on `jvmMain` only, by `qa`'s `FontspectorCliChecker` to parse a `fontspector --json` report as a generic `JsonElement` tree (P1-qa; no `@Serializable` classes of its own, so `qa` does not apply the `kotlin-serialization` compiler plugin) | 1.11.0 | Apache-2.0 | POM. Resolved via `:qa:corpus:dependencies` and `:qa:dependencies` (`jvmRuntimeClasspath` and `wasmJsRuntimeClasspath`): only `kotlinx-serialization-core`/`-core-jvm`/`-core-wasm-js` 1.11.0 beyond `kotlin-stdlib` on both platforms, the same 1.11.0 xmlutil's common-metadata variant already names above, so the two do not collide. `ui` depends on `qa`, which depends on `qa:corpus` (`api`), so this reaches the apps' runtime classpath even though none of the app modules names it directly |
